...
| 缺失项 | 影响 | 状态 |
|--------|------|:----:|
| 缺双盲标注一致性报告 | 无法量化标注歧义 | 🔜 后续引入 |
| 人工抽检覆盖率仅 10% | 可能遗漏低频边缘 case | 🔜 扩大至 30% |

> 📊 **标注质量报告已生成**：[quality_report/README.md](quality_report/README.md) — 含完整统计指标
> 🖼️ **可视化抽检**：[quality_report/visual_check/](quality_report/visual_check/) — 12 组 GT-vs-Image 对比
...
| 缺失项 | 状态 |
|--------|:----:|
| 无视觉难度标签（模糊度、遮挡度、字体大小） | 🔜 后续补充 |

> 🏷️ **多维难度标签已生成**：[quality_report/difficulty_labels.md](quality_report/difficulty_labels.md) — OCR 维度 + 视觉复杂度（文字密度、结构复杂度、参数值丰富度）三个新维度
> 📋 **test.jsonl 已更新**：每个样本均含 `difficulty` 字段（ocr_based + visual + 3 维子标签）
